...
function placeEnemies(tiles, num, level) {
  var type = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'enemy';

  var indices = placeObjectsOfType(type, tiles, num);
  if (type === 'boss') {
    return _defineProperty({}, indices, { hp: 250, dmg: 40 });
  }
...
var Game = React.createClass({
  ...
  createGameLevel: function createGameLevel(mapSize) {
    var dungeonNumber = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  ...
  fight: function fight(idx) {
    ...
    var _weapons$weapon$attac = _slicedToArray(weapons[weapon].attack, 2),
        minAttack = _weapons$weapon$attac[0],
        maxAttack = _weapons$weapon$attac[1];
    ...
    switch (tiles[idx].hasObject) {
      ...
      case 'enemy':
        var _fight = this.fight(idx),
            enemyHP = _fight.enemyHP,
            playerHP = _fight.playerHP;
